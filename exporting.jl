...
include("params.jl")
pars = Dict(zip(parnames, parvals))
reps = pars["replicates"][1]

means=zeros(nsets*reps, 8)

for i in 1:nsets
	for j in 1:reps
    		dum=load("data"*lpad(string(i),length(string(nsets)),"0")*"-"*lpad(string(j),length(digits(reps)),"0")*".jld2")
    		means[(i-1)*10+j,:] = [dum["params"].b, dum["params"].c, dum["params"].d, dum["params"].clink, mean(dum["typehist"][20000:100000]),  mean(dum["pn"][20000:100000]),  mean(dum["pr"][20000:100000]), j]
	end
end

CSV.write("means.csv", DataFrame(means, Symbol.(["b", "c", "d", "clink", "typehist", "pn", "pr", "rep"])))
